...
def jobs(q: str = "", ats_exclude: str = "", matched: bool = False,
    ...
    if closed_mode not in ("exclude", "only", "any"):
        closed_mode = "exclude"
    ats_excl = [a.strip() for a in ats_exclude.split(",") if a.strip()] if ats_exclude else None
    rows, total = db.list_jobs(
        q=q or None, ats=ats_excl, matched_only=matched,
...
_pdflatex_ok: bool | None = None


def _pdflatex_available() -> bool:
    global _pdflatex_ok
    if _pdflatex_ok is None:
        import shutil as _sh
        _pdflatex_ok = bool(_sh.which("pdflatex"))
    return _pdflatex_ok
...
def job_jd(job_id: int):
    ...
    if not job:
        raise HTTPException(404, "job not found")
    ctx = _resume.get_job_context(job, companies=_companies_for_detect())
    ctx["pdflatex"] = _pdflatex_available()
    return ctx


def _safe_filename(name: str, role: str) -> str:
    import re as _re
    def part(s: str) -> str:
        s = _re.sub(r"[^A-Za-z0-9_-]+", "_", (s or "").strip())
        return s.strip("_") or "resume"
    return f"{part(name)}_{part(role)}.pdf"


@app.post("/api/resume/build")
def build_resume(form: dict = Body(...)):
    if not _pdflatex_available():
        raise HTTPException(
            422, "LaTeX (pdflatex) is not available in this environment. "
                 "Rebuild the image with the TeX Live layer (see Dockerfile).")
    try:
        tex = _resume.render_tex(form)
        pdf = _resume.compile_pdf(tex)
    except _resume.ResumeCompileError as e:
        raise HTTPException(422, f"LaTeX compile failed: {e}")
    except _resume.ResumeError as e:
        raise HTTPException(422, str(e))
    except Exception as e:
        raise HTTPException(500, f"resume build failed: {e}")
    name = form.get("name") or "resume"
    role = form.get("jobTitle") or form.get("role") or "resume"
    fname = _safe_filename(name, role)
    return Response(
...
def stats():
    ...
    try:
        ...
        pass
    try:
        s["dead_boards"] = db.dead_board_count()
    except Exception:
        pass
    try:
        sw = db.get_sweep()
        total = sw.get("sweep_total") or 0
        covered = sw.get("sweep_covered") or 0
        sw["pct"] = round(100.0 * covered / total, 1) if total else 0.0
        s["sweep"] = sw
    except Exception:
        pass
    return s
...
@app.get("/api/demand")
def demand(limit: int = Query(200, ge=1, le=1000)):
    return db.skill_demand(limit)
